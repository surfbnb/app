...
import {Image, TouchableOpacity, Text, View, Share, Platform, KeyboardAvoidingView, SafeAreaView} from 'react-native';
        ...
        <KeyboardAvoidingView behavior={Platform.OS == 'android' ?'padding' :''} style={{ flex: 1 }} keyboardVerticalOffset={30}>
          ...
          <WebView
            ...
            renderError={(errorName) => (
              <View style={inlineStyles.webviewContent}>
                <Text style={{ fontSize: 20 }}>Invalid Link</Text>
              </View>
            )}
...
const BrowserHeaderRight = (props) => {
  ...
  return <BrowserMenu url={url} share={shareDelegate} reload={reloadDelegate} />;
};
  ...
  return (
    <View style={[inlineStyles.footerStyles]}>
      <TouchableOpacity onPress={props.goBack} style={inlineStyles.navigateWrapper}>
        <Image
          style={[inlineStyles.navigateSkipFont, !props.canGoBack ? { tintColor: Colors.lightGrey } : '']}
          source={BackActive}
        ></Image>
      </TouchableOpacity>
      <TouchableOpacity onPress={props.goForward} style={inlineStyles.navigateWrapper}>
        <Image
          style={[inlineStyles.navigateSkipFont, !props.canGoForward ? { tintColor: Colors.lightGrey } : '']}
          source={ForwardActive}
        ></Image>
      </TouchableOpacity>
    </View>
  );
