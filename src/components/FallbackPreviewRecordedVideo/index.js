...
class PreviewRecordedVideo extends PureComponent {
  constructor(props) {
    ...
    this.nextUrlComponentOne =  this.videoUrlsList[1] || {};
    ...
    this.seekCount = 0;
    this.appStateTimeOut = 0;
    console.log('this.videoUrlsList.length', this.videoUrlsList.length);
  ...
  handleProgressZero = (progress) => {
    console.log('nextUrlComponentZero progress', progress.currentTime);
    if(progress.currentTime < 0.75){
      this.nextUrlComponentOne = this.getNextVideoUri();
      console.log('nextUrlComponentOne', this.nextUrlComponentOne);
    }
    this.handleProgress(progress);
  };

  handleProgressOne = (progress) => {
    console.log('nextUrlComponentOne progress', progress.currentTime);
    if(progress.currentTime < 0.75){
      this.nextUrlComponentZero = this.getNextVideoUri();
      console.log('nextUrlComponentZero', this.nextUrlComponentZero);
    }
    this.handleProgress(progress);
  };
  ...
  showVideoComp = () => {
    console.log(this.state.previewURI, 'this.state.previewURIthis.state.previewURIthis.state.previewURIthis.state.previewURI');

    const isShowingComponent0 = true ; //this.isCurrentActiveComponentZero();
    ...
    return <View style={{flex: 1}}>
      <Video
        ...
        style={{
          ...
          width: "100%",
          height: "100%",
          zIndex: isShowingComponent0 ? 1  : 0,
          flex: isShowingComponent0 ? 1 : 0
        }}
        onPlaybackRateChange={this.onPlaybackRateChange}
        posterResizeMode={'cover'}
        resizeMode={'cover'}
        onProgress={this.handleProgressZero}
      ...
      {/*    width:  "100%",*/}
      {/*    height:"100%",*/}
      {/*    zIndex:isShowingComponent1 ? 1 : 0,*/}
      {/*    flex: isShowingComponent1 ? 1 : 0*/}
      {/*  }}*/}
      {/*  onPlaybackRateChange={this.onPlaybackRateChange}*/}
      {/*  posterResizeMode={'cover'}*/}
      {/*  resizeMode={'cover'}*/}
      {/*  onProgress={this.handleProgressOne}*/}
  ...
  onPlaybackRateChange = (args) => {
    ...
    console.log(args, 'playRate:::playRate');
    /*
    ...
    * */
    if (playRate === 0 && this.currentTime > 0 && !this.isPaused() ){
      console.log(args.playbackRate, 'args.playbackRate');
      this.seekCount++;
      if (this.seekCount <= 8 ) {
  ...
  handleEnd = () => {
    ...
    if (this.videoUrlsList.length - 1 === this.state.index ) {
  ...
  getNextVideoUri = () => {
    ...
    return this.videoUrlsList[this.indexOfVideo + 1] || this.videoUrlsList[0] ;
  };
